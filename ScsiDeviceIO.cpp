...
std::string GetDvdDriveLetter()
{
    ...
    if ( dwResult > 0 && dwResult <= MAX_PATH )
    {
        ...
        while( *szSingleDrive )
        {
            const UINT driveType = GetDriveType( (LPCWSTR)szSingleDrive );
...
int GetDvdStatus(QString drive)
{   
//    const std::string dvdDriveLetter = GetDvdDriveLetter();
    const std::string dvdDriveLetter = drive.toStdString();
...
//    if( iResult == 1 )  return 2;   
    ...
    hDevice = CreateFile( (LPCWSTR)strDvdPath.c_str(),
    ...
    if(iResult)  
    {     
		std::cout << "Drive is: " << dvdDriveLetter.c_str() << std::endl;
		std::cout << "DataBuf is: " << DataBuf << std::endl;
